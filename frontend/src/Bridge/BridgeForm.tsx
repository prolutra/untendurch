...
const BridgeForm: FC<BridgeFormProps> = observer(({ bridgeFormState }) => {
  ...
  const saveBridge = async (event: React.FormEvent) => {
    ...
    const persistedFiles = await uploadFiles(persistedBridge.id, state.images);
    ...
    <Grid gap={0} columns={[1, '1fr 1.6181fr 1fr']}>
      ...
      <Flex sx={{ position: 'relative' }}>
        ...
        >
          ...
          <Flex sx={{ flexDirection: 'column', gap: 3 }}>
            ...
            <Flex>
              ...
              <Input
                ...
                type={'file'}
                accept=".jpg,.jpeg,.png"
                sx={{ width: 0, height: 0, opacity: 0, overflow: 'hidden' }}
          ...
          <Flex sx={{ gap: 3, mb: 5, alignItems: 'center' }}>
            {state.objectId && (
